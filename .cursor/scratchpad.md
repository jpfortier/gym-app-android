...
- **304 handling (Mar 2026):** Client changes for GET /prs/{id}/image 304 Not Modified: (1) GymApi.prImage accepts optional If-None-Match header. (2) PrImageLoader caches (etag, bytes) per prId; on subsequent requests sends If-None-Match; on 304 returns cached bytes. (3) ETag extracted from response.raw().priorResponse (302) or main response. (4) Unit test PrImageLoaderTest verifies 304→cached path.
